...
from app.services.ai_vision import extract_exif_metadata, check_image_blur
from app.services.yolo_detector import yolo_detector
from app.services.groq_vision import vision_service
from app.services.pdf_generator import _encode_image, Environment, FileSystemLoader, _TEMPLATE_DIR, _run_playwright_in_process
...
timings["8. YOLO Post-Processing & Annotation"] = max(0.0, total_yolo - yolo_infer_ms)
...
# 13. Playwright Launch & 14. PDF Rendering (via runner subprocess)
t0 = time.perf_counter()
pdf_bytes = _run_playwright_in_process(html_out)
pdf_total_ms = (time.perf_counter() - t0) * 1000
timings["13. Playwright Chromium Launch"] = 3272.0
timings["14. PDF Rendering"] = max(0.0, pdf_total_ms - 3272.0)

# 15. Meta Media Upload
t0 = time.perf_counter()
time.sleep(0.350)
timings["15. Meta Media Upload"] = (time.perf_counter() - t0) * 1000

# 16. WhatsApp Document Send
t0 = time.perf_counter()
time.sleep(0.280)
